/** Procedural HDR-style cubemap (magenta / cyan / yellow / blue) from bubble2. */
...
function noise3(x: number, y: number, z: number) {
  ...
  const sample = (a: number, b: number, c: number) => hash3(ix + a, iy + b, iz + c);
...
function envColor(
  ...
  const hue = (uv[0]! * 2.5 + uv[1]! * 1.8 + bands * 0.35 + t * 0.08) % 1;
  const col = spectral(hue);
...
function encodeHdr(rgb: [number, number, number]) {
  return rgb.map((c) => {
...
export function createEnvCubemap(
  gl: WebGL2RenderingContext,
  { size = 128, time = 0, intensity = 1 } = {},
) {
