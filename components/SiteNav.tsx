...
export default function SiteNav({ className = "" }: SiteNavProps) {
  ...
  return (
    <nav
      className={`hero-nav-entrance pointer-events-none fixed inset-x-0 top-0 z-50 flex items-start justify-between px-[clamp(1.5rem,3.1vw,3rem)] pt-[clamp(1.75rem,2.5vw,2.25rem)] ${
        entranceReady ? "hero-nav-entrance--visible" : ""
      } ${className}`}
      aria-label="Primary"
    >
